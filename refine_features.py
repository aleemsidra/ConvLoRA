...
def train_target(dataset_train, dataset_train_dice, dataset_val, config, suffix, wandb_mode, initial_lr=0.001, level=0, device=torch.device("cuda:0")):
    ...
    alpha_batch = config.alpha_batch
    alpha_layer = config.alpha_layer
    bn_level = config.bn_level
    ...
    #  # Iterate over the modules in the ResBlock2d
    # for name, module in model.init_path.named_modules():

    #     # Check if the module is a PreActivationND layer
    #     if isinstance(module, PreActivationND):

    #         # Set the requires_grad attribute of the BatchNorm layer to True
    #         print("name", name, "module", module)
    #         module.bn.requires_grad = True
    #         updated_bn_layers.append(name + '.bn')
        # embed()
        # for module in model.init_path.modules():

        # for module in model.modules():
            # if isinstance(module, PreActivationND):
            #     for params in module.bn.parameters():
            #         params.requires_grad = True
                # module.bn.weight.requires_grad = True
                # module.bn.bias.requires_grad = True
           
        #     elif isinstance(module, nn.BatchNorm2d):
        #         for param in model.batchnorm.parameters():
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            update_bn_momentum = bn_momentum * np.exp(-alpha_batch * batch_idx)
            # for module in model.init_path.modules():
            
            if bn_level == "batch": 
                for module in model.modules():
                    if isinstance(module, PreActivationND):
                #         # batch shrinking based on batch 
                        module.bn.momentum = update_bn_momentum
                    elif isinstance in (module, nn.BatchNorm2d):
                        # update_bn_momentum = bn_momentum * np.exp(-alpha_batch * batch_idx)
                        module.momentum = update_bn_momentum

            elif bn_level == "layer": 
                print("bn_level : layer")
                depth_of_bn_layer = 1
                # for name, layer in model.init_path.named_modules(): # for init_path update
                for name, layer in model.named_modules():
                    if isinstance(layer, nn.BatchNorm2d):
                
                        layer.momentum =  bn_momentum * \
                                                np.exp(-alpha_layer * depth_of_bn_layer)
                        depth_of_bn_layer += 1
            ...
            if bn_level is not None:
                print("donot need optimizer following UBAN")
                pass
            else:
        ...
        with torch.no_grad():
            ...
            print('old val dice', train_dice_total_avg_old)
            ...
            print(f'Epoch: {epoch}, Train DC: {avg_train_dice}, Valid Loss, {val_loss_total_avg}, Valid DC: {avg_val_dice}')
